...
def add_structures_to_data(data_tuple, input_dir) ->tuple:
    ...
    for name in names:
        ...
        if ct_file_path:
            structures.append(ct2dbn(ct_file_path))
        else:
            structures.append(0)  # or handle the case where the file is not found

    return (names, sequences, structures)

def data_split(data_tuple, cluster_df, test_size=0.2, random_state=42):
    """
...
def main():
    ...
    # train, test split
    train_set, test_set = data_split(data, cluster_df, test_size=0.2, random_state=42)
